...
Clicking on the corresponding strip of the NIAC opens a new window. The first tab provides more details, while the second tab gives an overview of the policies related to the NIAC:
...
>[!ATTENTION]
> To add an alias, enter the name and press **Enter or the spacebar** **_before_** clicking Submit.

> [!WARNING]
> A device that is connected to a NIAC will not be able to reach the internet unless the NIAC is a member of a group for which the WAN Gateway is enabled. For instance: if a NIAC is a member of the group WanGateway and WAN GATEWAY is enabled for that group in Group Configuration, then the connected device will be able to reach the internet. Detailed instructions about Group Configuration are available [here](./rules/groupconfiguration/groupconfiguration.md).
