...
1. [Install Jimber SASE Client on the Domain Controller](#install-network-isolation-on-the-domain-controller)
   ...
   - [Installation](#installation)
     ...
     - [Configuring dns forwarding for SASE server](#configuring-dns-forwarding-for-SASE-server)
     - [Configuring dns forwarding for the domain controller](#configuring-dns-forwarding-for-the-domain-controller)
2. [SASE Server Configuration](#SASE-server-configuration)
   - [Port configuration on SASE server](#port-configuration-on-SASE-server)
...
![Static RPC Ports](./screenshots/ad-static-rpc-ports.png ':size=500')

- #### Set DNS Manager Listening Interface 

> [!IMPORTANT]
> Secure Mode Only - Breaks Native Network Domain Communication Functionality!
...
The installer can be found [here](https://SASE.jimber.io/clients/windows-server-latest.msi). The installation is done by simply executing the MSI as an administrator.

Starting Jimber SASE, a dialog box will emerge: 

![jimber_server_settings.png](./screenshots/jimber_server_settings.png ':size=400')

The required token can be retrieved from the 'Servers' tab within the SASE Server Interface. Simply select and edit the  correct server to find it.
...
![Restarting the Service](./screenshots/ad-restarting-the-service.png ':size=500')
...
- #### Configuring DNS forwarding for SASE Server
...
![DNS Properties](./screenshots/ad-dns-properties.png ':size=400')

It is also possible to configure the forwarding DNS servers here: 
    
![DNS Forwarders](./screenshots/ad-dns-forwarders.png ':size=300')
