...
Within the ***_<span style="color: darkblue;">Jimber SASE Platform</span>_***, you can create and manage user device scripts under the **User Device Scripts** section. These scripts are executed on the user's device and support two types of triggers:

- **On Connect** – triggered when the device connects to the ***_<span style="color: darkblue;">Jimber SASE Platform</span>_*** via the **_Jimber SASE Client_**.  
...
![Screenshot of the User Device Scripts page](./screenshots/scripts.png ':size=700')
...
To create a new Script into the platform, click on the `+ Create new` button and follow the steps below:

![Screenshot of the Create User Device Script window](./screenshots/create_script.png ':size=300')


1. **Name**: Enter a descriptive name for the script to help identify its purpose.
2. **Select Group**: Choose the user group this script should apply to. The script will only be executed for users who are members of this group.
> [!ATTENTION]
...
6. **Script Field**: Write or paste your script content in the provided editor. Make sure it matches the requirements of the target platform:
> [!ATTENTION]
>- .bat or .ps1 for Windows-based systems
>- .sh (Bash) for Linux/macOS-based systems
7. **Activate Changes**: Press the `Create` button to save and apply the new user device script.
...
> [!TIP]
> You can refresh the list of scripts by pressing the refresh icon ![](../../images/icons/icon_reload.png ':size=20') at the top of the page.


### User Device Scripts Details

Clicking on the corresponding strip of a script opens a detailed view with additional information.
